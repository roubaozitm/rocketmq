...
public class DefaultMessageStore implements MessageStore {
    ...
    // CommitLog文件转发请求（ConsumeQueue文件、Index 索引文件等）
    ...
    public boolean load() {
        ...
        try {
            // 临时的abort文件是否存在，若文件若在，证明上次退出不正常
    ...
    public void start() throws Exception {
        ...
        // ConsumeQueue文件刷盘任务启动
        this.flushConsumeQueueService.start();
        // Commitlog文件启动（包含启动各种Commitlog相关的任务）
    ...
    public PutMessageResult putMessage(MessageExtBrokerInner msg) {
        ...
        // 消息主题长度超过127个字符将拒绝该消息写入
        if (msg.getTopic().length() > Byte.MAX_VALUE) {
            log.warn("putMessage message topic length too long " + msg.getTopic().length());
            return new PutMessageResult(PutMessageStatus.MESSAGE_ILLEGAL, null);
        }

        // 消息属性长度超过32767个字符将拒绝该消息写入
                ...
                if (bufferConsumeQueue != null) {
                    try {
                        ...
                        for (; i < bufferConsumeQueue.getSize() && i < maxFilterMessageCount; i += ConsumeQueue.CQ_STORE_UNIT_SIZE) {
                            ...
                            // 说明之前有通过物理偏移量从commitlog中获取数据未获取到，将nextPhyFileStartOffset重置为了未获取
                            // 到数据的物理偏移量所在文件的下一个文件
                            if (nextPhyFileStartOffset != Long.MIN_VALUE) {
                                // 此次解析的数据单元的物理偏移量小于该nextPhyFileStartOffset值，则跳过下面的处理逻辑，继续解析下一个数据单元
                            ...
                            // 检查此次读取的数据量是否大于了阀值，若大于则停止继续解析，跳出此次遍历
                            if (this.isTheBatchFull(sizePy, maxMsgNums, getResult.getBufferTotalSize(), getResult.getMessageCount(),
                                isInDisk)) {
                                break;
                            }

                            // 根据SubscriptionData对象和数据块中的tagsCode进行消息过滤
                            ...
                            if (null == selectResult) {
                                ...
                                /*
                                 * 获取该物理偏移量offsetPy所在文件的下一个commitlog文件的起始偏移量。因为以该offsetPy为读取开始位置未
                                 * 获取到数据说明对应的commitlog文件被删除了，应该更新下一次读取位置值nextPhyFileStartOffset等于该被删
                                 * 文件的下个文件的起始偏移量
                                 */
...
//                    }
    ...
    private boolean isTheBatchFull(int sizePy, int maxMsgNums, int bufferTotal, int messageTotal, boolean isInDisk) {
        ...
        if (isInDisk) {
            // 若是从磁盘中读取数据：一次被拉取的消息字节数不得大于1024 * 64，一次被拉取的消息个数不得大于8
            ...
            // 若是从内存中读取数据：一次被拉取的消息字节数不得大于1024 * 256，一次被拉取的消息个数不得大于25
    ...
    public void putMessagePositionInfo(DispatchRequest dispatchRequest) {
        // 通过topic和queueId信息，获得ConsumeQueue文件对象
    ...
    class CleanCommitLogService {
        ...
        private void deleteExpiredFiles() {
            ...
            // 如果该文件被其他线程所占用，会阻止此次任务，destroyMapedFileIntervalForcibly表示第一次拒绝删除之后能保留的最大时间
            int destroyMapedFileIntervalForcibly = DefaultMessageStore.this.getMessageStoreConfig().getDestroyMapedFileIntervalForcibly();

            // 获取指定删除文件的时间点，默认为凌晨4点
    ...
    class ReputMessageService extends ServiceThread {
        ...
        private void doReput() {
            ...
            for (boolean doNext = true; this.isCommitLogAvailable() && doNext; ) {
                ...
                // 是否开启重复转发，如果开启检测更新位置小于commitlog确认位置
                if (DefaultMessageStore.this.getMessageStoreConfig().isDuplicationEnable()
                    && this.reputFromOffset >= DefaultMessageStore.this.getConfirmOffset()) {
                    break;
                }

                // 从commitlog读取更新位置的日志，返回reputFromOffset偏移量开始的全部有效数据
                        ...
                        for (int readSize = 0; readSize < result.getSize() && doNext; ) {
                            ...
                            if (dispatchRequest.isSuccess()) {
                                if (size > 0) {
                                    ...
                                    // 构建ConsumeQueue完成之后，reputFromOffset增加，下次从这后面继续构建
                                    ...
                                    // 消息长度为0，将下个文件的第一个消息设置为下个reput的消息
