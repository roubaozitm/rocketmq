 ...
 *
 * Commitlog文件存储目录为${ROCKET_HOME}/store/commitlog，每一个文件默认1G，一个文件写满后再创建另外一个，
 * 以该文件中第一个偏移量为文件名，偏移量小于20位用0补齐，这样根据物理偏移量能快速定位到消息。
 */
public class CommitLog {
    ...
    public void recoverAbnormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            ...
            while (true) {
                ...
                if (dispatchRequest.isSuccess()) {
                    ...
                    if (size > 0) {
                        ...
                        // 分发commitlog的消息到consumerqueue文件和index文件
            ...
            // 清除ConsumeQueue文件的废弃数据，从processOffset开始后面的数据都需要废弃
            if (maxPhyOffsetOfConsumeQueue >= processOffset) {
                log.warn("maxPhyOffsetOfConsumeQueue({}) >= processOffset({}), truncate dirty logic files", maxPhyOffsetOfConsumeQueue, processOffset);
                this.defaultMessageStore.truncateDirtyLogicFiles(processOffset);
            }
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            ...
            if (msg.getDelayTimeLevel() > 0) {
                ...
                // 用延迟消息主题SCHEDULE TOPIC、消息队列ID更新原先消息的主题与队列ID
                msg.setTopic(topic);
        ...
        // 获取当前可以写入的Commitlog文件映射
        MappedFile unlockMappedFile = null;
        MappedFile mappedFile = this.mappedFileQueue.getLastMappedFile();

        // 加锁，将消息存储到CommitLog文件中是串行的。此处的锁类型根据配置决定是自旋锁（CAS）还是重入锁（ReentrantLock）
        putMessageLock.lock();
        try {
            ...
            // 将消息追加到CommitLog文件缓存中
            result = mappedFile.appendMessage(msg, this.appendMessageCallback);
            switch (result.getStatus()) {
                // 追加成功
                case PUT_OK:
                    break;
                // CommitLog存储不足，创建新的CommitLog，重新追加
                case END_OF_FILE:
                    // 后面会在内存中解锁原来的commitlog文件映射
    ...
    /**
     * 刷盘
     * @param result           刷文件映射结果
     * @param putMessageResult 刷文件结果
     * @param messageExt       消息
     */
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            if (messageExt.isWaitStoreMsgOK()) {
                // 构建同步刷盘任务GroupCommitRequest，参数为消息的物理偏移量
        ...
        // 异步刷盘
            ...
            if (!this.defaultMessageStore.getMessageStoreConfig().isTransientStorePoolEnable()) {
                // 未开启TransientStorePool，唤醒刷盘服务（文件映射->文件）
                flushCommitLogService.wakeup();
            } else {
                // 开启TransientStorePool，唤醒提交服务（堆内存->文件映射->文件）
     ...
     * @param offset
     * @return offset - (offset % mappedFileSize) + mappedFileSize
     */
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                try {
                    ...
                    // result为false表示有数据提交，更新时间并唤醒刷盘服务
                    if (!result) {
    ...
    class FlushRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                // 获取运行方式，true表示使用sleep方式，false表示使用唤醒方式
                ...
                // 当前时间超过两次真正刷盘的最大时间间隔，此次必须进行刷盘（flushPhysicQueueLeastPages置为0）
    ...
    class GroupCommitService extends FlushCommitLogService {
        ...
        /**
         * 由于避免同步刷盘消费任务与其他消息生产者提交任务直接的锁竞争，GroupCommitService
         * 提供读容器与写容器，这两个容器每执行完一次任务后，交换，继续消费任务
         */
        ...
        public void run() {
            ...
            synchronized (this) {
                // 交换容器，提交永远在requestsWrite中，刷盘时永远读取requestsRead。使用两个变量的原因是为了刷盘不阻塞提交
                this.swapRequests();
            }
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // 初始化存储IP地址和端口的byteBuffer
            this.resetByteBuffer(hostHolder, 8);
            // 获取消息ID，4字节IP，4字节端口，8字节消息偏移
            String msgId = MessageDecoder.createMessageId(this.msgIdMemory, msgInner.getStoreHostBytes(hostHolder), wroteOffset);

            // 构造key(topicName-queueId)，获取该消息在队列的偏移
            ...
            // 超过文件的大小要求，只存储总大小和魔数，返回状态为END_OF_FILE
            ...
            // 消息体内容，长度为bodyLenth中存储的值
            if (bodyLength > 0)
                this.msgStoreItemMemory.put(msgInner.getBody());
            // 16 TOPIC 主题存储长度，1字节
            this.msgStoreItemMemory.put((byte) topicLength);
            // 主题，长度为TopieLength中存储的值
            this.msgStoreItemMemory.put(topicData);
            // 17 PROPERTIES 消息属性长度，2字节
            this.msgStoreItemMemory.putShort((short) propertiesLength);
            // 消息属性，长度为PropertiesLength中存储的值
            if (propertiesLength > 0)
